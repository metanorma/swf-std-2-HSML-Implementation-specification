...
* **Identification**: An `act:ActivityStep` instance is uniquely identified by its URI. It **SHOULD** also have a simple, locally-unique `dct:identifier` to simplify referencing within the workflow definition.
...
| **Usage Note** | While the step's URI provides global uniqueness, this identifier provides a simple, stable "nickname" (e.g., "stepA", "validateUser") that **SHOULD** be used to uniquely identify the step *within its parent `act:CompositeActivitySchema`*. This makes it easier for `act:DataLink` instances to reliably reference the step as a source or target without using long, complex URIs.
|===
